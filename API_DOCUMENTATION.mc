...
Base URL: `https://credibot-api-production-e42d.up.railway.app/api/v1`
...
async function getClientes(page = 1, perPage = 25) {
  const response = await fetch(
    `https://credibot-api-production-e42d.up.railway.app/api/v1/clientes?page=${page}&per_page=${perPage}`
  );
...
async function getClienteById(id) {
  const response = await fetch(
    `https://credibot-api-production-e42d.up.railway.app/api/v1/cliente/${id}`
  );
  if (!response.ok) {
...
async function smartChat(message) {
  const response = await fetch(
    'https://credibot-api-production-e42d.up.railway.app/api/v1/smart-chat',
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
- **HTTPS Obrigatório**: Railway força HTTPS, sempre use `https://`
- **Rate Limiting**: Sem limite atual, mas use com moderação
- **Timeout**: Requisições podem levar até 30s (especialmente smart-chat)
- **Ambiente**: Produção no Railway, sem ambiente de desenvolvimento público
